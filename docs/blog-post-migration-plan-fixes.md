# WordPress Blog Migration — Stage 1.1 (Dump Refresh + Slug Fixes)

## Status

Stage 1 (data extraction) is **already implemented and run** — `scripts/migrate-blog/` (constants, sql-parser, db, mojibake, markdown, images, html-extractor, serialize, index) exists and produced the current `data/blog.ts` (44 posts, 674 comments, images/avatars under `public/old-blog/`). That pipeline's design is documented in the "Original Stage 1 design" section at the bottom of this file for reference — it does not need to be re-read to execute this amendment, only re-run with the changes below.

## This amendment — why

Three issues surfaced after Stage 1 ran:

1. **The SQL dump had baked-in mojibake** (CP850 misdecoding of UTF-8 accented characters). The user has since regenerated a corrected dump: `project_archive/devhouse-wordpress/db/devhouse_backup_20260624_174219.sql` — verified directly: `está` now reads correctly (no `est├í`), same table prefix (`ab3f44oy3_`) and table structure as before (posts still chunked into 4 `INSERT` statements, comments in 1), so the existing parser needs no logic changes, just a path update.
2. **Post slug "143"** — WordPress fell back to the numeric post ID as the slug because the title (`<img title vs alt | ...`) had no sluggable text. The user wants a real slug derived from the title via `slugify`, generalized to any post whose slug is purely numeric (not hardcoded to "143"), plus a standalone CLI script for ad-hoc future use.
3. **Bilingual slugs** — the site is bilingual (`en`/`pt`) and every other text field on `BlogPost` (`title`/`excerpt`/`content`) is already `Record<Lang, string>` with `en: ""` as a translate-later placeholder. `slug` should follow the same convention for consistency and to avoid a breaking schema change later when English slugs are added.

## Changes to make

### 1. Point the parser at the corrected dump
In `scripts/migrate-blog/constants.ts`, update `SQL_DUMP_PATH` to `devhouse_backup_20260624_174219.sql`. No other parsing logic changes — `mojibake.ts`'s fix is idempotent/harmless on already-clean text, so it stays in place as a safety net rather than being removed.

### 2. Add `slugify` + numeric-slug fallback
- Add `slugify` as a devDependency — same package/version `semprecomvoce-next` uses (`slugify@^1.6.9`), confirmed usage pattern there: `slugify(title, { lower: true, strict: true })` (`scripts/gen-post-data.js:112`).
- Add a small standalone CLI utility, e.g. `scripts/slugify.ts`, that takes a title string as an argv arg and prints the slug — wired into `package.json` as `"slugify": "tsx scripts/slugify.ts"` so it's usable ad hoc from the shell (`pnpm slugify "Some Title"`) for any future post, independent of the migration pipeline.
- In the migration pipeline (`html-extractor.ts`'s `enumerateSlugs`/`extractPost`, or `index.ts`), detect any post whose WordPress `post_name` matches `/^\d+$/` (purely numeric — generalizes beyond just "143") and regenerate its **output** slug from the (entity-decoded) title via the same `slugify(title, { lower: true, strict: true })` call. The original numeric folder name (e.g. `"143"`) must still be used to locate the HTML file on disk (`rip/site/143/index.html`) — only the slug written into `data/blog.ts` changes.
- Because the internal-link rewriter (`markdown.ts`'s `extractSlugFromHref` + `knownSlugs` set in `html-extractor.ts`) matches link targets by folder/slug name, build a **folder-name → final-slug** map once after this renaming, and resolve through it both when constructing `knownSlugs` and when checking an extracted href's folder name — so any future link pointing at a renamed numeric-slug post still correctly resolves to `post:<new-slug>`. (Checked directly: nothing in the current corpus actually links to `/143/`, so this doesn't change today's output, but the fix should be general, not a one-off hack, since the dump may grow other numeric-slug posts later.)

### 3. Bilingual slug schema
In `serialize.ts`'s emitted `BlogPost` interface, change:
```ts
slug: string;
```
to:
...
```
In `index.ts`'s post-object construction, change `slug: extracted.slug` to `slug: { pt: extracted.slug, en: "" }` (matching the existing `title`/`excerpt`/`content` pattern exactly). No change needed to the `post:<slug>` internal-link scheme itself — every WordPress-era link only ever targets the `pt` slug (the old site had no English version), so `post:<slug>` continues to encode the bare slug string; Stage 2's resolver will look it up as `BLOG_POSTS.find(p => p.slug.pt === slug)`.

### 4. Re-run
`pnpm migrate:blog` regenerates `data/blog.ts` and `public/old-blog/` fully from source (idempotent, as before). Re-verify: 44 posts, mojibake-free text (spot check a few accented titles/bios), post "143"'s slug is now title-derived (not numeric), and `slug` fields are now `{ pt, en: "" }` objects.

## Out of scope (discussed, not planned here)

Syntax highlighting / Markdown rendering in React (the user's question 4) is Stage 2 rendering work, not part of this data-layer amendment — discussed conversationally with the user, to be designed in a future planning session once they're ready to touch the React components.

---

## Original Stage 1 design (for reference — already implemented)
