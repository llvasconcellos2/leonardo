/* global React, POSTS, T, Kicker, PostCard */
// ── LV Portfolio · blog reading view + writing index ────────────────
function BlogPost({ id, lang, go }) {
  ...
  return (
    <article className="lv-read">
      <button className="lv-link-arrow lv-back" onClick={() => go('writing')}>
      ...
      <div className="lv-read-meta">
        <span>{post.date}</span><span>{post.read} {t.minRead}</span><span>PT-BR · EN</span>
      </div>
      <div className="prose lv-read-body">
        <p className="lv-read-lede">{post.excerpt[lang]}</p>
        {body.map((para, i) => <p key={i}>{para}</p>)}
        <pre className="lv-code-block"><code><span className="cm">// crawl + rewrite + self-host</span>{'\n'}<span className="kw">const</span> snap = <span className="kw">await</span> crawl(url, {'{'} rewriteBase: <span className="tr">true</span> {'}'});</code></pre>
        <p>{lang === 'pt' ? 'É assim que 70 sistemas mortos continuam contando a história.' : 'That is how 70 dead systems keep telling the story.'}</p>
      </div>
    </article>
  );
}

function WritingIndex({ lang, go }) {
  const t = T[lang];
  return (
...
Object.assign(window, { BlogPost, WritingIndex });
